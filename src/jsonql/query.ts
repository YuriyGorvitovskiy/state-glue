...
export type ISubQuery<R> = {
    ...
    : key extends '$type' ? string
    : key extends '$page' ? IPage | 'all'
    : key extends '$aggregate' ? boolean
    : key extends '$dir' ? 'forward' | 'reverse'
    : key extends '$field' ? string
    : key extends '$cmp' ? 'any' | 'exist' | 'not exist'
    : key extends '$return' ? boolean
    : key extends keyof R ? ISubQuery<R[key]> | IField | value
    : void
};
...
const injectChildrenDefaults = <R>(aggregate: boolean, select: ISelect<R>): ISelect<R> => {
    return Object.fromEntries(Object.entries(select)
        ...
        .map(([k, v]) => [k, injectChildDefaults(k as keyof R, aggregate, v)])) as ISelect<R>;
}

const injectChildDefaults = <R, T extends ISubQuery<R> | IField | value>(k: keyof R, aggregate: boolean, v: T): ISubQuery<R> | IField => {
